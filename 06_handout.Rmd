...
### Simplify names and create dates
...
### Convert age to integer and simplify names even more
...
We will no pose questions and answer them with graphs. In session, we may pose different questions, we will see.
...
### Seasonality of (secondary) sex ratio at birth
...
Plot it as a ridge plot! This is a new `geom` for us. `geom_ridgeline()` comes from the `ggridges` package. The main mappings are `x`, `y`, and `height`, where `x` is straightforward, `y` refers to the level of each ridge baseline, and `height` refers to `y` value of each individual distribution.
```{r}
density_by_mother_age %>% 
...
3. `age_diff < -2`
Recreate the first ridgeplot (using `mother_age5`), and map `fill` color to your new 3-category `age_diff`. You'll want to set `alpha` so that the densities aren't fully overlapped.
...
### Exercise: (5 min)
1. Make a surface plot of `p_weeks` by `birth_weight` (try `geom_density2d_filled()`)
Hint: tabulate the data first, and maybe try rounding `birth_weight` to intervals of just 0.1?
