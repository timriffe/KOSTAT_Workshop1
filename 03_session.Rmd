...
Note, I give a more rigorous but complicated approach to this issue at the end of this tutorial! This is a simple version, whose primary value is in seeing how one can programmatically rescale like this. We later check and note that the age pattern and each of our synthetic indices later calculated (TFR or MAB) was basically unphased after taking the rigorous approach.
...
Here used to recode our age classes:
```{r, warning = FALSE}
# handle cases:
# P %>% pull(age) %>% unique()
P <- 
...
We do this by getting *this year* January 1 next to *next year* Jan 1, using `lead()`, then averaging to approximate a July 1 population.
```{r}
a <- 0:10
a
lead(a)
...
P <-
  ...
  # lead() needs to happen inside groups!! Otherwise the shift
  # would cross between populations!!
...
Here are all the exposure calculation steps in a single annotated pipeline:
```{r, message = FALSE}
E <- 
  read_excel(path = "Data/demo_pjan.xlsx",
             range = "A10:CZ510",
...
`R` note: it would be cool to do `group_by()` then `mutate()` to move the 5-year data to single ages, but `mutate()` requires us to keep the same number of rows, so that won't work. `group_modify()`, can handle this. To apply it, our function (we see how to write them Monday!) needs to take a `tibble` and return a `tibble`, so it's written in that way. The objects `K8` and `S` are instrumental to the method used. 
```{r}
...
B_infer_cs <-
  ...
  group_by(country, year) %>% 
  group_modify(~graduate_CS_df(data = .x, 
                               S = S, 
                               K8 = K8)) %>% 
  ungroup()
```
