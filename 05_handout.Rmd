...
title: | 
  ...
  | \vspace{0.3cm} \huge \textbf{Day 5: Visualizing data using ggplot2}\vspace{0.6cm}
  | 
fontsize: 11pt
...
output:
  ...
  header-includes:
    ...
    - \fancyhead[LO]{{Day 5: Visualizing data using ggplot2}}
...
The purpose of graphics in demographic research is varied. We us visualization in diagnostic ways. Such plots needn't be perfect, they just need to reveal the desired diagnostic check discernibly well. These should be quick to make and interpret. Other plots are exploratory in nature, also quick, but flexible. `ggplot2` functions make such explorations particularly easy. Finally there are presentation graphics, which aim to distill a clear message from the data, and where storeytelling and detail management matters. 
...
*NOTE: It's super common to have a pipeline run straight into a plot, and sometimes we forget to switch from `%>%` to `+` after the `ggplot()`. Happens all the time, but quick to fix.
...
gapminder %>% 
  ...
  scale_y_log10()
```
I use `scale_x_log10()` because the default tick positions/labels are nice. 
...
By default, a loess smoother is used for smaller datasets, a GAM smoother for larger, but others are available to you, see `?geom_smooth` for more options. These smoothers are in fact doing some statistics on top of the data, but only for the purposes of plotting the result as an enhancement of the legibility of the plot, just to be clear on that point. 
...
Currently our plot has lots of overlapping points. We probably have very dense areas, but can't really know which is the densest, because the points are all pushed together. This is called overplotting, and it reduces the information of the plot in an undesirable way. There are some easy things we can do to offset this effect. We could deal with this by setting points to be semi-transparent, using an argument called `alpha` in this case, we might not want to *map* a variable to alpha (although we could-- I did it in session 3...), but rather just set a level for all points. To do this, we'll want to specify `alpha` (a number between 0 and 1) outside the `aes()`, directly inside the relevant `geom`:
