...
We will compare mortality in Taiwan and Japan. I downloaded their mortality rates from the HMD and combined them into a single tidy dataset. I save you having to replicate that code and have posted the data as a `csv` on the github site. You can read it directly into `R`, below.
...
Japan has a higher CDR than Taiwan. On the surface it may seem as if Japan had *higher mortality* than Taiwan. However, if we look at the age-specific death rates, we have a different story.
...
For example, when applied to the CDR (using `J` and `T` for Japan and Taiwan), the decomposition is written as:
...
M_Dec %>% 
  ...
  summarize(RE = sum(RE) * 1000,
            ...
            CDR_Taiwan = sum(M_Taiwan * Sx_Taiwan) * 1000) %>% 
...
Let's select just the columns we'll need, and move them side by side, like before. Except, rather than typing `Japan` and `Taiwan` so many times.... let's just type `j` and `t`?

```{r}
LT_arriaga <-
  LT %>% 
  mutate(Country = if_else(Country == "Japan","j","t")) %>% 
...
LT_arriaga <-
  LT_arriaga %>% 
  mutate(direct = lx_t * (nLx_j / lx_j - nLx_t / lx_t),
         indirect = lead(Tx_j) * 
           (lx_t / lx_j - 
              lead(lx_t) / lead(lx_j)),
