...
title: | 
  ...
  | \vspace{0.3cm} \huge \textbf{Day 3: Population Growth}\vspace{0.6cm}
  | 
fontsize: 11pt
...
As usual we will use tidy coding where possible, but this isn't always possible for things that are iterative or involve matrix algebra, so I'll need to introduce new programming concepts today. Note the concepts around population growth or reproductivity discussed here are often misinterpreted and misused for nationalist and racist agendas. We're smarter than that and understand that these are just partially useful models. These are conceptual models, not forecasts, and many simplifying assumptions are applied when modeling population growth. 

# Data

We will use the Spanish data from the HMD [@HMD] and HFD [@HFD] today. Please load the following datasets straight from GitHub using this code (I will paste this in the Google Doc as well:
...
B_url <- "https://raw.githubusercontent.com/timriffe/KOSTAT_Workshop1/master/Data/BirthsSpain2014.txt"
B2014 <- read_csv(B_url, 
                  show_col_types = FALSE) 
# Death counts in 2014
D_url <- "https://raw.githubusercontent.com/timriffe/KOSTAT_Workshop1/master/Data/DeathsSpain2014.txt"
...
P1_url <- "https://raw.githubusercontent.com/timriffe/KOSTAT_Workshop1/master/Data/PopulationSpain2014.txt"
P2014 <- read_csv(P1_url,
                  show_col_types = FALSE) 

# Population counts on January 1st, 2015
P2_url <- "https://raw.githubusercontent.com/timriffe/KOSTAT_Workshop1/master/Data/PopulationSpain2015.txt"
P2015 <- read_csv(P2_url,
                  show_col_types = FALSE) 
...
* Note: the value 2.1 is a rule of thumb, since it's based on survival it's a moving variable. The lower infant and child mortality is, the closer this value approaches 2. This threshold value can be calculated for a given population, typically by (as usual) abusing the period synthetic population assumption, see e.g. @espenshade2003surprising 
...
* All relationships are time-reversible with an altenative interpretation.
