...
for (i in 1:60){
  ...
  read_csv(this_path,
    ...
    select(location, measure = rei, sex, age, year, prev = val) %>% 
    write_csv(path_out)
}
```
...
vroom::vroom(file.path(gbd_folder,dir(gbd_folder)),
                  show_col_types = FALSE) %>% 
  filter(age != "All ages") %>% 
  rename(age_label = age) %>% 
  left_join(gbd_ages, by = "age_label") %>% 
  select(location, measure, sex, year, age, prev) %>%
  write_csv("Data/gbd_share.csv.gz")
```
